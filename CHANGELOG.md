...
- メルカリ側の商品ページ構造変更（2026-09-09、「商品の編集」リンクの `data-testid` が `checkout-link` から `checkout-button` に変更）により、すべてのアイテムで編集ページに遷移できず値下げが止まる問題を修正（編集ページへの `href` でリンクを特定するよう変更）
- 化粧品カテゴリなどで編集ページに表示される「法令に基づく表示事項を登録しました」同意チェックボックスが未チェックのため送信できず、該当アイテムが毎回失敗していた問題を修正（送信前に自動でチェックを入れる）
