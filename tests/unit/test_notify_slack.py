...
def _build_page() -> unittest.mock.MagicMock:
    """スクリーンショットとして 1x1 の PNG を返すモック Page"""
    page = unittest.mock.MagicMock()
    img = PIL.Image.new("RGB", (1, 1), color="red")
    buf = io.BytesIO()
    img.save(buf, format="PNG")
    page.screenshot.return_value = buf.getvalue()
    page.content = "<html><body>Test</body></html>"
    return page
...
class TestErrorWithScreenshot:
    ...
    def test_calls_error_with_image(self, mock_page, slack_config):
        ...
        with unittest.mock.patch("my_lib.notify.slack.error_with_image") as mock_error:
            mercari_bot.notify_slack.error_with_screenshot(
                slack_config, "テストエラー", "エラー詳細", mock_page
            )
    ...
    def test_with_empty_slack_config(self, mock_page):
        """SlackEmptyConfig でも動作する"""
        empty_config = SlackEmptyConfig()
        with unittest.mock.patch("my_lib.notify.slack.error_with_image") as mock_error:
            mercari_bot.notify_slack.error_with_screenshot(empty_config, "タイトル", "メッセージ", mock_page)

            mock_error.assert_called_once()
            assert mock_error.call_args[0][0] == empty_config

    def test_screenshot_is_pil_image(self, mock_page, slack_config):
        """スクリーンショットが PIL.Image として渡される"""
        with unittest.mock.patch("my_lib.notify.slack.error_with_image") as mock_error:
            mercari_bot.notify_slack.error_with_screenshot(slack_config, "タイトル", "メッセージ", mock_page)
...
class TestErrorWithTraceback:
    ...
    def test_includes_traceback(self, mock_page, slack_config):
        """traceback が含まれる"""
        with unittest.mock.patch("my_lib.notify.slack.error_with_image") as mock_error:
            try:
                raise ValueError("テスト例外")
            except ValueError:
                mercari_bot.notify_slack.error_with_traceback(slack_config, "エラー発生", mock_page)
    ...
    def test_title_is_passed(self, mock_page, slack_config):
        """タイトルが渡される"""
        with unittest.mock.patch("my_lib.notify.slack.error_with_image") as mock_error:
            try:
                raise RuntimeError("dummy")
            except RuntimeError:
                mercari_bot.notify_slack.error_with_traceback(slack_config, "カスタムタイトル", mock_page)
...
class TestDumpAndNotifyError:
    ...
    def test_calls_dump_page(self, mock_page, slack_config, tmp_path):
        """dump_page が呼ばれる"""
        with (
            unittest.mock.patch("my_lib.browser.helpers.dump_page") as mock_dump,
            unittest.mock.patch("my_lib.browser.helpers.clean_dump"),
            unittest.mock.patch("my_lib.notify.slack.notify_error_with_page"),
        ):
            exc = RuntimeError("test")
            mercari_bot.notify_slack.dump_and_notify_error(slack_config, "エラー", mock_page, tmp_path, exc)
    ...
    def test_calls_clean_dump(self, mock_page, slack_config, tmp_path):
        """clean_dump が呼ばれる"""
        with (
            unittest.mock.patch("my_lib.browser.helpers.dump_page"),
            unittest.mock.patch("my_lib.browser.helpers.clean_dump") as mock_clean,
            unittest.mock.patch("my_lib.notify.slack.notify_error_with_page"),
        ):
            exc = RuntimeError("test")
            mercari_bot.notify_slack.dump_and_notify_error(slack_config, "エラー", mock_page, tmp_path, exc)
    ...
    def test_calls_notify_error_with_page(self, mock_page, slack_config, tmp_path):
        ...
        with (
            ...
            mercari_bot.notify_slack.dump_and_notify_error(
                slack_config, "カスタムタイトル", mock_page, tmp_path, exc
            )
    ...
    def test_dump_path_is_pathlib(self, mock_page, slack_config, tmp_path):
        ...
        with (
            ...
            exc = RuntimeError("test")
            mercari_bot.notify_slack.dump_and_notify_error(slack_config, "エラー", mock_page, dump_path, exc)

            assert mock_dump.call_args[0][2] == dump_path

    def test_with_empty_slack_config(self, mock_page, tmp_path):
        ...
        with (
            ...
            exc = RuntimeError("test")
            mercari_bot.notify_slack.dump_and_notify_error(empty_config, "エラー", mock_page, tmp_path, exc)
    ...
    def test_dump_failure_does_not_block_notification(self, mock_page, slack_config, tmp_path):
        """ダンプに失敗しても通知は行われる（ブラウザ死亡時）"""
        with (
            unittest.mock.patch("my_lib.browser.helpers.dump_page", side_effect=RuntimeError("dead")),
            unittest.mock.patch("my_lib.notify.slack.notify_error_with_page") as mock_notify,
        ):
            mock_page.screenshot.side_effect = RuntimeError("dead")
            type(mock_page).content = unittest.mock.PropertyMock(side_effect=RuntimeError("dead"))

            mercari_bot.notify_slack.dump_and_notify_error(
                slack_config, "エラー", mock_page, tmp_path, RuntimeError("test")
            )

            mock_notify.assert_called_once()
            assert mock_notify.call_args[0][3] is None
            assert mock_notify.call_args[0][4] is None
