...
├── demo.py                         # モック実行デモ
└── mercari_bot/
    ├── __main__.py                 # python -m mercari_bot 用エントリポイント
    ├── cli.py                      # docopt CLI・プロファイル巡回
    ├── config.py                   # 設定読み込み・型定義（dataclass）
    ├── exceptions.py               # 例外階層
    ├── history.py                  # 値下げ履歴の記録・売却検知（SQLite）
    ├── logic.py                    # 純粋ロジック（パース・値下げ幅決定）
    ├── mercari_price_down.py       # 値下げ処理の中核
    ├── notify_slack.py             # ページダンプ付きエラー通知
    └── progress.py                 # Rich 進捗表示・observer
```

### 処理フロー

1. `cli.py`: 設定読み込み → 各プロファイルに対して `mercari_price_down.execute()` を実行 →
   失敗プロファイル数を集計して終了コード（0/1）を決定
2. `mercari_price_down.py`:
    ...
    - `_execute_item()` で各アイテムの値下げ処理（結果を `ItemResult` で返す）
    - 各アイテムの処理結果を `history.HistoryDb` に記録（`-D` 時は `NullHistoryDb`）
    - 走査完了後、前回スナップショットとの差分から売却（取り下げ）を検知して Slack 通知

### 値下げロジック

- 更新から一定時間経過したアイテムのみ対象（`interval.hour` で設定）
- お気に入り数に応じて値下げ幅を決定（`discount` で設定、favorite_count 降順に設定読み込み時ソート済み）
- 最低価格（`threshold`）を下回る場合はスキップ
- 価格は 10 円単位に丸める
- 送信（edit-button クリック）後の TimeoutException は `PriceVerificationTimeoutError`
  （`PostSubmitError` 系）に変換し、my_lib のリトライに乗せない（検証スキップの防止）
...
- docopt を使用
- エントリポイントは `src/mercari_bot/cli.py`（console script `mercari-bot`、`python -m mercari_bot` も可）
- スキーマパスは `cli.py` で `__file__` 基準に解決（CWD 非依存）
...
- `data.history`（省略可）で値下げ履歴 DB のパスを指定（省略時は `data.selenium` 配下の `history.db`）
- パース時に運用ミス（threshold < 300、favorite_count: 0 のデフォルト条件欠如）を警告ログで検出
...
- Google Chrome 同梱
- `uv` でパッケージ管理
- エントリポイント: `mercari-bot -l`

### Kubernetes

`kubernetes/` に CronJob 設定（1日2回実行：7:00, 18:00）
